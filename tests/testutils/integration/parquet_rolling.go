...
func (cfg *Test) testParquetRolling(ctx context.Context, t *testing.T, testTable string) error {
	...
	if err := testutils.UpdateSelectedStreams(cfg.TestConfig, cfg.TestConfig.Namespace, "", "", []string{testTable}, cfg.TestConfig.ColumnToExclude); err != nil {
	...
	cfg.TestConfig.ExecuteQuery(ctx, t, cfg.TestConfig, "clean")
	cfg.TestConfig.ExecuteQuery(ctx, t, cfg.TestConfig, "rolling_seed")
	...
	if code, out, err := testutils.RunOlake(ctx, cfg.TestConfig, testutils.SyncArgs(false, parquetDestinationFile)...); err != nil || code != 0 {
...
func (cfg *Test) verifyParquetRolling(t *testing.T, table string) {
	t.Helper()
	ctx := t.Context()
